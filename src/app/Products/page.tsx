'use client'; // This is necessary for making this file a Client Component
...
export default function ProductsPage() {
  ...
  const [filter, setFilter] = useState({ men: false, women: false });
      ...
      <div className="w-full sm:w-1/4 p-5">
        ...
        <ul className="product-ul font-medium py-5 border-b border-gray-300">
          {[
            ...
            <li className="pt-1 px-12 font-[500] text-[15px]" key={item}>
              {item}
            </li>
          ))}
        </ul>
          ...
          {filteredProducts.map((product) => (
            <div key={product.productName} className="p-4">
              <Link href={`/ProductDetails/${product.slug.current}`}>
                <div>
                <Image
             src={product.imageUrl}  // Ensure this is a valid URL
